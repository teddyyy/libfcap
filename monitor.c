							...
							MACADDR_TYPE(addr), u_int8_t channel)
{
	...
	while (switch_count > 0) {
		...
		h80211[len++] = switch_count;		// switch channel counter
									...
									struct mif *mi, int rate, int rssi, int subtype)
{
	if (len >= 24) {
		...
		case 0x00:
			...
			break;
		case 0x20:
			handle_reassoc_request(mfn, pkt, len, rate, rssi);
			break;
		case 0x40:
			handle_probe_request(mfn, pkt, len, rssi);
			break;
		case 0xA0:
			handle_disassoc_request(mfn, pkt, len, rssi);
			break;
		case 0xB0:
			handle_auth_frame(mfn, pkt, len, rssi);
			break;
		case 0xC0:
			handle_deauth_frame(mfn, pkt, len, rssi);
			break;
		case 0x08:
			handle_data_frame(mfn, pkt, len, rate, rssi);
			break;
		}
	}
...
void mon_free(struct monitor_fn_t *mfn)
{
	struct ap_conf *ap = (struct ap_conf *)mfn->mfn_priv;
	ap_free(ap);
}
